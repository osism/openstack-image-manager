...
# digest algorithms implied by the length of a bare hex digest
_ALGO_BY_LENGTH = {32: "md5", 40: "sha1", 64: "sha256", 128: "sha512"}


def parse_checksum(
    checksum: typing.Optional[str],
) -> typing.Optional[typing.Tuple[str, str]]:
    """Split a '<algo>:<hex>' or bare '<hex>' digest into (algo, hex).

    A bare digest's algorithm is inferred from its length. Returns None for a
    missing or malformed digest or an unsupported algorithm.
    """
    if not checksum:
        return None
    if ":" in checksum:
        ...
        digest = checksum
        algo = _ALGO_BY_LENGTH.get(len(digest), "")
    if algo not in _ARIA2_ALGO or not re.fullmatch(r"[0-9a-fA-F]+", digest):
        return None
...
def checksum_to_aria2(checksum: typing.Optional[str]) -> typing.Optional[str]:
    ...
    if parsed is None:
        return None
    algo, digest = parsed
    return f"{_ARIA2_ALGO[algo]}={digest}"
...
class ImageManager:
    ...
    def create_cli_args(
        ...
        verify_checksum: bool = typer.Option(
            False,
            "--verify-checksum",
            help="Verify every image against its checksum, downloading it via "
            "aria2 + glance-direct; fail imports that cannot be verified",
        ),
    ):
    ...
    def get_checksum_from_checksums_url(self, url: str, checksums_url: str) -> str:
        ...
        "<digest> <filename>", "<digest> *<filename>" or
        "<ALGO> (<filename>) = <digest>"; the line naming the image file is used.
        ...
        for line in checksums_file.splitlines():
            # "<digest> <name>", "<digest> *<name>" or "<ALGO> (<name>) = <digest>"
            tokens = line.replace("(", " ").replace(")", " ").split()
            if not any(token.lstrip("*") == filename for token in tokens):
                continue
            for token in tokens:
                if self.is_checksum(token):
                    return token
        return ""
    ...
    def import_image(
        ...
        Params:
            ...
            checksum: optional '<algo>:<hex>' or bare '<hex>' digest, used to
                verify the image wherever the manager downloads or reads it
        """
        ...
        if parsed_checksum is None:
            if self.CONF.verify_checksum:
                logger.error(
                    f"Cannot verify image {name}: no usable checksum ({checksum!r})"
                )
                self.exit_with_error = True
                return None
            logger.warning(f"Image {name} has no usable checksum to verify it against")

        parsed_url = urllib.parse.urlparse(url)
        if parsed_url.scheme == "file":
            if parsed_checksum is not None:
                algo, expected = parsed_checksum
                try:
                    actual = file_digest(parsed_url.path, algo)
                except OSError as e:
                    logger.error(f"Failed to read local file for image {name}\n{e}")
                    self.exit_with_error = True
                    return None
                if actual != expected:
                    logger.error(
                        f"Checksum mismatch for image {name}: {algo} of "
                        f"'{parsed_url.path}' is {actual}, expected {expected}"
                    )
                    self.exit_with_error = True
                    return None
                logger.info(f"Verified {algo} checksum of image {name}")
        ...
        # prefetch=always or --verify-checksum: skip web-download entirely, as
        # only a download made by the manager can be verified
        if self.CONF.prefetch == "always" or self.CONF.verify_checksum:
            return self._prefetch_import(properties, name, url, checksum, deadline)

        if parsed_checksum is not None:
            logger.warning(
                f"Checksum of image {name} is not verified by a web-download import"
            )
    ...
    def _download(
        ...
        if result.returncode == 32:
            # aria2c's exit status for a failed checksum validation
            logger.error(f"Checksum mismatch for {url}, expected {checksum}")
            return False
                ...
                if not self.CONF.dry_run:
                    import_result = self.import_image(
                        ...
                        checksum=(
                            upstream_checksum
                            if version == "latest"
                            else versions[version].get("checksum")
                        ),
                    )
                    if import_result:
                        logger.info(
