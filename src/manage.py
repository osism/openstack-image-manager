...
class ImageManager:
    ...
    def main(self) -> None:
        ...
        for image in all_images:
            ...
            if imported_image is not None and image['multi']:
    ...
    def import_image(self, name: str, image: dict, url: str) -> None:
        '''
        ...
        while True:
            try:
                status = self.conn.image.get_image(new_image).status
                if status != 'active':
    ...
    def process_image(self, image: dict, versions: dict, sorted_versions: list) -> tuple:
        ...
        params:
            image - image dict from images.yml
        returns:
            a set with all existing images
        '''
        ...
        for version in sorted_versions:
            ...
            if not existence and not (self.CONF.latest and len(sorted_versions) > 1 and version != sorted_versions[-1]):
                ...
                if not self.CONF.dry_run:
                    self.import_image(name, image, url)
    ...
    def manage_outdated_images(self, managed_images: set) -> None:
        '''
