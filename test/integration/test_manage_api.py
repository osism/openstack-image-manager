...
class TestManageAPI(TestCase):
    ...
    def test_api_functions_for_web_download(self):
        ...
        imported_image = self.sot.import_image(
            self.web_image,
            self.web_image_name,
        ...
        # test set properties
        self.web_image["meta"]["image_build_date"] = self.web_image["versions"][0][
            "build_date"
        ]
        ...
        self.assertEqual(
            image.hw_watchdog_action, self.web_image["meta"]["hw_watchdog_action"]
        )
        ...
        self.assertEqual(
            image.properties["image_original_user"], self.web_image["login"]
        )
        self.assertEqual(image.properties["internal_version"], "1")
        self.assertEqual(
            image.properties["replace_frequency"],
            self.web_image["meta"]["replace_frequency"],
        )
        self.assertEqual(
            image.properties["uuid_validity"], self.web_image["meta"]["uuid_validity"]
        )
        self.assertEqual(
            image.properties["provided_until"], self.web_image["meta"]["provided_until"]
        )
        self.assertEqual(
            image.properties["image_build_date"],
            str(self.web_image["versions"][0]["build_date"]),
        )
        ...
        self.assertEqual(
            cloud_images,
            {},
            f"Could not delete image {self.web_image['name']}, please do so manually",
        )
    ...
    def test_api_functions_for_file_upload(self):
        ...
        # test set properties
        self.file_image["meta"]["image_build_date"] = self.file_image["versions"][0][
            "build_date"
        ]
        self.sot.set_properties(
            self.file_image,
        ...
        self.assertEqual(
            image.hw_watchdog_action, self.file_image["meta"]["hw_watchdog_action"]
        )
        ...
        self.assertEqual(
            image.properties["image_original_user"], self.file_image["login"]
        )
        self.assertEqual(image.properties["internal_version"], "1")
        self.assertEqual(
            image.properties["replace_frequency"],
            self.file_image["meta"]["replace_frequency"],
        )
        self.assertEqual(
            image.properties["uuid_validity"], self.file_image["meta"]["uuid_validity"]
        )
        self.assertEqual(
            image.properties["provided_until"],
            self.file_image["meta"]["provided_until"],
        )
        self.assertEqual(
            image.properties["image_build_date"],
            str(self.file_image["versions"][0]["build_date"]),
        )
        self.assertFalse(image.is_hidden)
        ...
        self.assertEqual(
            cloud_images,
            {},
            f"Could not delete image {self.file_image['name']}, please do so manually",
        )
