...
class TestManageAPI(TestCase):
    ...
    def test_api_functions(self):
        ...
        imported_image = self.sot.import_image(
            self.image, self.name, self.image["versions"][0]["url"]
        )
        ...
        self.assertEqual(
            image.hw_watchdog_action, self.image["meta"]["hw_watchdog_action"]
        )
        ...
        self.assertEqual(
            cloud_images,
            {},
            f"Cloud not delete image {self.image['name']}, please do so manually",
        )
