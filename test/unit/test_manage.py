...
FAKE_IMAGE_DICT = {
    ...
    "visibility": "public",
    "multi": True,
    "meta": {"os_distro": "ubuntu", "os_version": "20.04"},
    "tags": [],
    "versions": [{"version": "1", "url": "http://url.com", "checksum": "1234"}],
}
...
class TestManage(TestCase):
    ...
    def test_process_image(
        self, mock_get_images, mock_requests, mock_import_image, mock_set_properties
    ):
        ...
        result = self.sot.process_image(
            self.fake_image_dict, self.versions, self.sorted_versions
        )
        ...
        mock_import_image.assert_called_once_with(
            self.fake_image_dict, self.fake_name, self.fake_url
        )
        mock_set_properties.assert_called_once_with(
            self.fake_image_dict, self.fake_name, self.versions, "1", ""
        )
        self.assertEqual(
            result,
            (
                {self.fake_image_dict["name"]},
                mock_get_images.return_value.__getitem__(),
                None,
            ),
        )
    ...
    def test_main(
        ...
        mock_process_image.assert_called_once_with(
            self.fake_image_dict, self.versions, ["1"]
        )
